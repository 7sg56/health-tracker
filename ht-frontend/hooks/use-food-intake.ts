...
export function useFoodIntake(options: UseFoodIntakeOptions = {}) {
  ...
  const searchFoodSuggestions = useCallback((query: string, limit: number = 10): FoodSuggestion[] => {
    if (!query.trim()) {
  ...
  const getFoodSuggestionsByCategory = useCallback((category: string): FoodSuggestion[] => {
  ...
  const addFoodIntake = useCallback(async (data: FoodIntakeRequest): Promise<FoodIntake> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to add food intake';
      setOptimisticError(errorMessage);
      throw err;
    }
  }, [actions]);

  const updateFoodIntake = useCallback(async (id: number, data: FoodIntakeRequest): Promise<FoodIntake> => {
    setOptimisticError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update food intake';
      setOptimisticError(errorMessage);
      throw err;
    }
  }, [actions, state.items]);

  const deleteFoodIntake = useCallback(async (id: number): Promise<void> => {
    setOptimisticError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete food intake';
      setOptimisticError(errorMessage);
      throw err;
    }
  }, [actions, state.items]);
  ...
  const averageCaloriesPerMeal = useMemo(() => {
    return state.items.length > 0 ? Math.round(totalCalories / state.items.length) : 0;
