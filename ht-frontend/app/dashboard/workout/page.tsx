...
export default function WorkoutPage() {
  ...
  // Calculate totals
  const totalMinutes = todayWorkouts.reduce((sum, workout) => sum + workout.durationMin, 0);
  const totalCalories = todayWorkouts.reduce((sum, workout) => sum + (workout.caloriesBurned || 0), 0);
  const weeklyMinutes = weeklyWorkouts.reduce((sum, workout) => sum + workout.durationMin, 0);
  const weeklyCalories = weeklyWorkouts.reduce((sum, workout) => sum + (workout.caloriesBurned || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Track your exercise and fitness activities. Goal: {dailyGoal} min per day
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <Badge variant={dailyProgress >= 100 ? "default" : "secondary"} className="text-xs">
      ...
      <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : isTablet ? 'grid-cols-2' : 'grid-cols-4'}`}>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 border-green-200 dark:border-green-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{totalMinutes} min</div>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 border-blue-200 dark:border-blue-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{weeklyMinutes} min</div>
            <p className="text-xs text-muted-foreground mb-2">
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20 border-purple-200 dark:border-purple-800">
          ...
          <CardContent>
            ...
            <Badge variant={currentStreak >= 7 ? "default" : currentStreak >= 3 ? "secondary" : "outline"}>
              {currentStreak >= 7 ? "🔥 On Fire!" : currentStreak >= 3 ? "💪 Building" : "🌱 Starting"}
            </Badge>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-muted-foreground">Most Popular Activity</h4>
                {(() => {
                  const activityCounts = workouts.reduce((acc, workout) => {
                    acc[workout.activity] = (acc[workout.activity] || 0) + 1;
                    return acc;
                  }, {} as Record<string, number>);
                  const mostPopular = Object.entries(activityCounts).sort(([,a], [,b]) => b - a)[0];
                  return mostPopular ? (
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary">{mostPopular[0]}</Badge>
                      <span className="text-sm text-muted-foreground">{mostPopular[1]} times</span>
                    </div>
                  ) : (
              ...
              {/* Average Session */}
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-muted-foreground">Average Session</h4>
                {workouts.length > 0 ? (
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">
                      {Math.round(workouts.reduce((sum, w) => sum + w.durationMin, 0) / workouts.length)} min
                    </Badge>
                    <span className="text-sm text-muted-foreground">per workout</span>
                  </div>
                ) : (
                  <span className="text-sm text-muted-foreground">No data yet</span>
                )}
              ...
              <div className="space-y-2">
                ...
                {(() => {
                  ...
                  thisMonth.setDate(1);
                  const monthlyWorkouts = workouts.filter(w => new Date(w.date) >= thisMonth);
                  const monthlyMinutes = monthlyWorkouts.reduce((sum, w) => sum + w.durationMin, 0);
                  return monthlyMinutes > 0 ? (
                    <div className="flex items-center gap-2">
                      <Badge variant="default">{monthlyMinutes} min</Badge>
                      <span className="text-sm text-muted-foreground">{monthlyWorkouts.length} workouts</span>
                    </div>
                  ) : (
